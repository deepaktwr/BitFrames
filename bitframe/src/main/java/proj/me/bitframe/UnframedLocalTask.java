...
class UnframedLocalTask extends AsyncTask<Void, Integer, BeanResult> {
    ...
    @Override
    protected BeanResult doInBackground(Void... params) {
        ...
        try {
            beanResult = Utils.decodeBitmapFromPath(
                    ...
                    imageResult.getNextUnframedBean().getImageLink(), imageResult.getContext());
        } catch (FrameException e) {
            e.printStackTrace();
        }
        if(beanResult == null) beanResult = new BeanResult();
        beanResult.setBeanImage(imageResult.getNextUnframedBean());
        return beanResult;
    }
